...
%
%Note: You need to have run SystemDefinition_example first, and then
%SteadyStateFinder. Run this after you have the steady state values in
%SteadyState_save already calculated.
...
for i = 1:length(SteadyState_save(:, 1))
    if System_save(SteadyState_save(i,1),length(k)+1) ~= 1 && SteadyState_save(i,n+2) ~= 2 %Exclude oscillators and unstable solutions
        ...
        SteadyState_save(i, n+3) = cond_1;
        SteadyState_save(i, n+4) = cond_2;
